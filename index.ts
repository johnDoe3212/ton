...
async function main() {
    const wallet = Address.parse('YOUR_WALLET'); //put here your wallet
    const collection = Address.parse('COLLECTION_ADDRESS'); //put here collection address. you can check it on getgems

    const endpoint = await getHttpEndpoint({
        network: "mainnet", //for testnet replace "mainnet" with "testnet"
    });
